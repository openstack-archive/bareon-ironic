...
class _SSHPortForwardingAbstract(object):
    ...
    def _kill(self):
        terminator = ProcTerminator(self.proc)
        terminator()
...
class ProcTerminator(object):
    ...
    def __call__(self):
        poll_iterator = loopingcall.FixedIntervalLoopingCall(self._do_poll)
        action_iterator = loopingcall.DynamicLoopingCall(self._do_action)
        queue = [
            (poll_iterator, poll_iterator.start(self._poll_delay)),
            (action_iterator, action_iterator.start())]

        is_over = False
        for ctrl, result in queue:
            if is_over:
                ctrl.stop()
            try:
                result.wait()
