...
class BareonVendor(base.VendorInterface):
    ...
    def deploy_steps(self, task, **data):
        ...
        if http_method == 'GET':
            ssh_keys_task = _InjectSSHKeyStepRequest(task, driver_info)
            return {'action': ssh_keys_task()}

        steps_mapping = _DeployStepMapping()
        data = _VendorDeployStepsAdapter(data)
        try:
            request_cls = steps_mapping.name_to_step[data.action]
        except KeyError:
            raise RuntimeError(
                'There is no name mapping for task: {!r}'.format(type(self)))
        handler = request_cls.response_handler(
            task, driver_info, data.action_payload)
        handler()
...
class _VendorDeployStepsAdapter(_AbstractAdapter):
    action = action_payload = None
    status = status_details = None

    def __init__(self, data):
        super(_VendorDeployStepsAdapter, self).__init__(data)

        try:
            self._extract_last_action()
        except KeyError as e:
            raise bareon_exception.DeployProtocolError(
                details='Missing mandatory field: {}'.format(e),
                payload=self._raw)

    def _extract_last_action(self):
        try:
            data = self._raw['action']
        except KeyError:
            return

        self.action = data['name']
        self.action_payload = data.get('payload', {})
        self.status = data['status']
        self.status_details = data.get('status-details', '')
...
class _NodeDriverInfoAdapter(_AbstractAdapter):
    ...
    def _process(self):
        ...
        if self.ssh_port is not None:
            ...
            if not 0 < self.ssh_port < 65536:
                raise exception.InvalidParameterValue(
                    'driver_info: Invalid SSH port number({}) is outside of '
                    'allowed range.'.format('bareon_ssh_port'))
    ...
    def _validate_ssh_key(self):
        missing = []
        for target in (self.ssh_key, self.ssh_key_pub):
            try:
                open(target).close()
            except IOError as e:
                missing.append(e)

        missing = ['{0.filename}: {0.strerror}'.format(e) for e in missing]
        if missing:
            self._errors.append(
...
@six.add_metaclass(abc.ABCMeta)
class _AbstractDeployStepResult(object):
    def __init__(self, task, driver_info, payload):
        self.task = task
        self.driver_info = driver_info
