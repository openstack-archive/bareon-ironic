...
class BareonVendor(base.VendorInterface):
    ...
    def deploy_steps(self, task, **data):
        ...
        try:
            ...
            raise RuntimeError(
                'There is no name mapping for deployement step: '
...
class _AbstractDeployStepResult(_AbstractDeployStepHandler):
    ...
    def _handle_error(self):
        LOG.error('Deployment step "{}" have failed: {}'.format(
...
class _DeploymentStepFailResult(_AbstractDeployStepResult):
    def _handle(self):
        LOG.error('Bareon is failed during deployment')
        if self.step_info:
            LOG.error('Failure details:')
            LOG.error(pprint.pformat(self.step_info.payload))


class _DeploymentStepFailRequest(_AbstractDeployStepResult):
    """Dummy object required to provide mapping for reporting handler"""
    name = None
    result_handler = _DeploymentStepFailResult
